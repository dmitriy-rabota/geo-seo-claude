...
5. Generate GEO-TECHNICAL-AUDIT.docx with results
...
**Primary deliverable: a Word document (.docx), NOT a markdown file.**

Generate `GEO-Audit-[Domain].docx` using Node.js with the `docx` npm package
(already installed globally). Save to `/mnt/user-data/outputs/` and call
`present_files` so the user can download it.
